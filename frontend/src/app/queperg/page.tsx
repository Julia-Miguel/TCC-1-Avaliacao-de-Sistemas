...
interface QuepergInterface {
    ...
    pergunta: {
        descricao: string; // Atualizado de enunciado para descricao
        tipo: string; // Atualizado de tipos para tipo
    };
    created_at: string;
    updated_at: string;
}
            ...
            <table>
                ...
                <tbody>
                    {quepergs.map(queperg => (
                        <tr key={queperg.id}>
                            <td>{queperg.id}</td>
                            <td>{queperg.questionario.titulo}</td>
                            <td>{queperg.pergunta.descricao}</td> {/* Atualizado de enunciado para descricao */}
                            <td>{queperg.pergunta.tipo}</td> {/* Atualizado de tipos para tipo */}
                            <td>{formatDate(queperg.created_at)}</td>
                            <td>{formatDate(queperg.updated_at)}</td>
                            <td><Link href={`/perguntas/update/${queperg.id}`}>Atualizar</Link></td>
                            <td>
                                <button onClick={() => handleDeleteQueperg(queperg.id)}>
                                    Excluir
                                </button>
                            </td>
                        </tr>
